...
#### JWT 인증 과정
 ...
 6. 서버에서는 JWT를 검증하여 사용자 정보를 확인
 7. 사용자가 확인되면 요청한 처리 결과를 클라이언트에게 넘겨줌

    - 세션 : 서버에 저장되어 있으면서 클라이언트에 있는 세션 ID 와 비교해서 사용자 인증
    - 토큰 : 인증 시 필요한 모든 정보가 담겨있으며 특정 서버에 저장되지 않음
 (cookie-parser, jsonwebtoken package 활용)

  ![image](https://github.com/dlgusdn978/NodeJS-Study-Notes/assets/73580422/db3f28e2-5a7c-43a3-9b07-7de4dca0452f)
